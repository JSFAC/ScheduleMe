...
final class ScheduleMeDataStore: ObservableObject {
    ...
    func loadBookings() async {
        ...
        if let lastBookingsFetchAt, Date().timeIntervalSince(lastBookingsFetchAt) < 180, !bookings.isEmpty {
            return
        }
    ...
    func loadThreads(for userID: String?) async {
        ...
        if let lastThreadsFetchAt, Date().timeIntervalSince(lastThreadsFetchAt) < 60, !threads.isEmpty {
            return
        }
    ...
    private func shouldUseNearbyBusinessesCache(for coordinate: CLLocationCoordinate2D) -> Bool {
        ...
        guard age < 300 else { return false } // 5 minutes
