...
struct AccountView: View {
    ...
    private var notificationsSection: some View {
        ScheduleMeCard {
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(spacing: 0) {
                    notifRow(icon: "checkmark.circle.fill", label: "Booking confirmations", isOn: $notifyBooking)
                    Divider().padding(.leading, 46).overlay(ScheduleMeTheme.cardBorder)
                    notifRow(icon: "bubble.left.fill", label: "New messages", isOn: $notifyMessages)
                    Divider().padding(.leading, 46).overlay(ScheduleMeTheme.cardBorder)
                    notifRow(icon: "bell.badge.fill", label: "Booking reminders", isOn: $notifyReminders)
                    Divider().padding(.leading, 46).overlay(ScheduleMeTheme.cardBorder)
                    notifRow(icon: "tag.fill", label: "Promotions & deals", isOn: $notifyPromos)
                }
    ...
    private func notifRow(icon: String, label: String, isOn: Binding<Bool>) -> some View {
        HStack(spacing: 12) {
            Circle()
