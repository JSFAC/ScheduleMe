...
struct BrowseView: View {
    ...
    // Shared header + search bar used in both scroll and non-scroll layouts
    @ViewBuilder
    private var browseHeaderAndSearch: some View {
    ...
    var body: some View {
        NavigationStack {
            // Map mode: fixed layout so the header stays visible
            if selectedViewMode == .map {
                ...
                ScheduleMeScreen {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        // Results
                        if !hasLocation && !dataStore.isLoadingBusinesses {
                            LocationPromptCard()
                                .padding(.horizontal, 20)
