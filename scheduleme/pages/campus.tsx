  ...
  if (loading) return (
    <>
      ...
      <div className="min-h-screen pb-20 md:pb-0" style={{ paddingTop: 'calc(48px + env(safe-area-inset-top, 0px))', background: dm ? '#0a0a0a' : '#EDF5FF' }}>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="rounded-2xl overflow-hidden border" style={{ background: dm ? '#171717' : 'white', borderColor: dm ? '#262626' : '#f0f0f0' }}>
              <div className="animate-shimmer h-32 sm:h-40" />
              <div className="p-3 space-y-2">
                <div className="animate-shimmer rounded h-3 w-2/3" />
                <div className="animate-shimmer rounded h-2.5 w-1/2" />
                <div className="animate-shimmer rounded h-2.5 w-1/3" />
              </div>
            </div>
          ))}
        </div>
