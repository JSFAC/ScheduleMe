// @ts-nocheck
// pages/api/detach-payment-method.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!(await rateLimit(req, res, { max: 20, windowMs: 60_000, keyPrefix: 'detach-pm' }))) return;
  ...
  if (profile.stripe_payment_method_id === payment_method_id) {
    await supabase.from('profiles').update({ stripe_payment_method_id: null }).eq('id', user.id);
  }
