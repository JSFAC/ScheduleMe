// pages/api/upload-review-media.ts — upload + moderate review images
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!(await rateLimit(req, res, { max: 20, windowMs: 60_000, keyPrefix: 'review-media' }))) return;
  ...
  const { error: upErr } = await supabase.storage.from('review-media').upload(filePath, buffer, { contentType: file_type, upsert: true });
