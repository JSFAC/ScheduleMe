...
const BusinessLoginPage: NextPage = () => {
  ...
  useEffect(() => {
    ...
    // If coming from "Create Password" email link, show email form pre-filled
    if (router.query.setup === 'password') {
      setShowEmail(true);
      if (router.query.email) setEmail(decodeURIComponent(router.query.email as string));
    }
  }, [router.query.error, router.query.setup, router.query.email, router]);
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      if (biz) {
        ...
        // Not a business account — just sign out, do NOT delete their account
        // They may be a consumer who landed here by mistake
