...
const SignIn: NextPage = () => {
  ...
  const shellBg = providerIntent ? 'bg-neutral-950' : 'bg-neutral-50';
  const cardBg = providerIntent ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-neutral-200';
  const titleColor = providerIntent ? 'text-white' : 'text-neutral-900';
  const bodyCopyColor = providerIntent ? 'text-neutral-400' : 'text-neutral-500';
  const subtleColor = providerIntent ? 'text-neutral-500' : 'text-neutral-400';
  const controlBg = providerIntent ? 'bg-neutral-800 border-neutral-700 text-white hover:bg-neutral-700' : 'bg-white border-neutral-200 text-neutral-700 hover:bg-neutral-50';
  const tabShellBg = providerIntent ? 'bg-neutral-800' : 'bg-neutral-100';
  const activeTabBg = providerIntent ? 'bg-neutral-950 text-white shadow-[0_8px_24px_rgba(0,0,0,0.25)]' : 'bg-white text-neutral-900 shadow-sm';
  const inactiveTabColor = providerIntent ? 'text-neutral-400' : 'text-neutral-500';
  const dividerBorder = providerIntent ? 'border-neutral-800' : 'border-neutral-100';
  const inputClasses = providerIntent
    ? 'form-input bg-neutral-800 border-neutral-700 text-white placeholder:text-neutral-600'
    : 'form-input';
  ...
  if (sent) {
    return (
      <>
        ...
        <div className={`min-h-screen ${shellBg} flex items-center justify-center px-6 pt-20`}>
          <div className="text-center max-w-sm">
            ...
            <h1 className={`text-2xl font-bold mb-2 ${titleColor}`}>Check your email</h1>
            <p className={`mb-6 ${bodyCopyColor}`}>
              {showReset
                ? <>We sent a password reset link to <strong>{email}</strong>.</>
                : <>We sent a confirmation link to <strong>{email}</strong>. Click it to activate your account.</>}
            </p>
            {!showReset && (
              <p className={`text-xs mb-5 ${subtleColor}`}>
            ...
            {!showReset && (
              <div className="space-y-2 mb-4">
                ...
                {resendMsg && (
                  <p className={`text-xs ${bodyCopyColor}`}>{resendMsg}</p>
                )}
              </div>
            )}
  ...
  return (
    <>
      ...
      {providerIntent ? <BusinessNav /> : <Nav />}
      <div className={`min-h-screen ${shellBg} flex items-center justify-center px-6 pt-20 pb-16 relative overflow-hidden`}>
        {providerIntent && (
          <div
            aria-hidden="true"
        ...
        <div className="w-full max-w-sm animate-fade-up">
          <div className="text-center mb-8">
            {providerIntent && <span className="section-eyebrow mb-3 block">Provider Setup</span>}
            <h1 className={`text-2xl font-bold mb-1 ${titleColor}`}>
              {providerIntent
                ? tab === 'login' ? 'Welcome back' : 'Start your provider setup'
                : 'Welcome to ScheduleMe'}
            </h1>
            <p className={`text-sm ${bodyCopyColor}`}>
              {providerIntent
                ? tab === 'login'
                  ? 'Sign in with the same ScheduleMe account you use everywhere else, then continue provider setup.'
                  : 'Create one ScheduleMe account, then we will turn on your provider role and send you straight into the dashboard checklist.'
          ...
          <div className={`rounded-2xl border p-8 ${cardBg} ${providerIntent ? 'shadow-[0_24px_80px_rgba(0,0,0,0.42)]' : 'shadow-card'}`}>
            ...
            <div className={`flex rounded-xl p-1 gap-1 mb-6 ${tabShellBg}`}>
              {(['login', 'signup'] as const).map(t => (
                <button key={t} type="button"
                  ...
                  className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${tab === t ? activeTabBg : inactiveTabColor}`}>
                  {t === 'login' ? 'Log In' : providerIntent ? 'Create Account' : 'Sign Up'}
                </button>
              ))}
            </div>
            ...
            {error && (
              <div className={`rounded-xl px-4 py-3 text-sm mb-5 ${providerIntent ? 'bg-red-500/10 border border-red-500/20 text-red-300' : 'bg-red-50 border border-red-100 text-red-700'}`}>{error}</div>
            )}

            {!showEmail ? (
              <div className="space-y-3">
                <button onClick={handleGoogle}
                  className={`w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl border transition-colors text-sm font-semibold ${controlBg}`}>
                ...
                <button onClick={() => setShowEmail(true)}
                  className={`w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl border transition-colors text-sm font-semibold ${controlBg}`}>
                  <svg className={`h-5 w-5 flex-shrink-0 ${providerIntent ? 'text-neutral-500' : 'text-neutral-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                  </svg>
                  {tab === 'login' ? 'Continue with Email' : providerIntent ? 'Create account with Email' : 'Sign up with Email'}
                </button>

                <div className="relative my-2">
                  <div className="absolute inset-0 flex items-center"><div className={`w-full border-t ${dividerBorder}`} /></div>
                  <div className="relative flex justify-center"><span className={`${providerIntent ? 'bg-neutral-900 text-neutral-600' : 'bg-white text-neutral-400'} px-3 text-xs`}>or</span></div>
                </div>

                <div className="text-center">
                  <Link href={providerIntent ? '/' : ((next as string) || '/bookings')} className="text-sm text-accent hover:underline font-medium">
                    {providerIntent ? 'Back to consumer page →' : 'Continue as guest →'}
                  </Link>
                  <p className={`text-xs mt-1 ${subtleColor}`}>
                    {providerIntent ? 'Your consumer account and provider role stay connected on one login.' : 'No account needed to find a pro'}
                  </p>
                </div>
              </div>
            ) : (
              <form onSubmit={handleEmail} className="space-y-4">
                <button type="button" onClick={() => { setShowEmail(false); setShowReset(false); setError(null); }}
                  className={`flex items-center gap-1.5 text-sm mb-2 transition-colors ${providerIntent ? 'text-neutral-500 hover:text-neutral-300' : 'text-neutral-500 hover:text-neutral-800'}`}>
                ...
                {showReset ? (
                  <>
                    <p className={`text-sm mb-1 ${providerIntent ? 'text-neutral-400' : 'text-neutral-600'}`}>Enter your email and we'll send a reset link.</p>
                    <div>
                      <label className={`block text-sm font-medium mb-1.5 ${providerIntent ? 'text-neutral-400' : 'text-neutral-700'}`}>Email</label>
                    ...
                    <button type="button" onClick={() => setShowReset(false)}
                      className={`w-full text-center text-xs transition-colors ${providerIntent ? 'text-neutral-600 hover:text-neutral-400' : 'text-neutral-400 hover:text-neutral-600'}`}>
                      ← Back to log in
                    </button>
                  </>
                ) : (
                  <>
                    {tab === 'signup' && (
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className={`block text-sm font-medium mb-1.5 ${providerIntent ? 'text-neutral-400' : 'text-neutral-700'}`}>First name</label>
                          <input type="text" required className={inputClasses} placeholder="First name"
                            value={firstName} onChange={e => setFirstName(e.target.value)} />
                        </div>
                        <div>
                          <label className={`block text-sm font-medium mb-1.5 ${providerIntent ? 'text-neutral-400' : 'text-neutral-700'}`}>Last name</label>
                          <input type="text" required className={inputClasses} placeholder="Last name"
                    ...
                    <div>
                      <label className={`block text-sm font-medium mb-1.5 ${providerIntent ? 'text-neutral-400' : 'text-neutral-700'}`}>Password</label>
                      <input type="password" required className={inputClasses} placeholder="••••••••"
                    ...
                    {captchaRequired && (
                      <div className="pt-1 flex flex-col items-start w-full">
                        ...
                        {!captchaWidgetId && !captchaLoadError && <p className={`mt-2 text-xs ${bodyCopyColor}`}>Captcha loading… If it doesn’t appear, disable ad blockers and refresh.</p>}
                        {captchaLoadError && <p className="mt-2 text-xs text-red-500">{captchaLoadError}</p>}
                        {captchaLoadError && (
                          <p className={`mt-1 text-xs ${bodyCopyColor}`}>
                    ...
                    {tab === 'login' && (
                      <div className="text-center">
                        <button type="button" onClick={() => setShowReset(true)}
                          className={`text-xs transition-colors ${providerIntent ? 'text-neutral-500 hover:text-accent' : 'text-neutral-400 hover:text-accent'}`}>
                          Forgot password?
                        </button>
                      </div>
                    )}
          ...
          <p className={`text-center text-xs mt-5 ${subtleColor}`}>
