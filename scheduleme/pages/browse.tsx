    ...
    <button onClick={onClick} className="biz-card group w-full text-left flex flex-col animate-fade-up"
      ...
      <div className="relative flex-shrink-0 w-full" style={{ aspectRatio: '4/3', background: dm ? '#2c2c2e' : '#e5e7eb' }}>
        ...
        <div className="absolute top-2.5 left-2.5">
          <div className="flex items-center gap-1.5 rounded-full px-2.5 py-1" style={{ background: 'rgba(0,0,0,0.52)', backdropFilter: 'blur(8px)' }}>
        ...
        {biz.price_tier && (
          <div className="absolute top-2.5 right-2.5">
            <div className="rounded-full px-2 py-1" style={{ background: 'rgba(0,0,0,0.52)', backdropFilter: 'blur(8px)' }}>
...
const BrowsePage: NextPage = () => {
  ...
  // Dynamic: only show categories that exist in loaded businesses
                  ...
                  {paginated.map(biz => (
                    <button key={biz.id} onClick={() => setActiveBiz(biz)}
                      ...
                      <div className="relative flex-shrink-0 overflow-hidden rounded-xl bg-neutral-100" style={{ width: 110, height: 130 }}>
                        ...
                        {(() => { const ls = getOpenStatus(biz.hours); return (
                          <div className="absolute top-2 left-2 flex items-center gap-1.5 rounded-full px-2 py-0.5" style={{ background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(6px)' }}>
                            <span className={`h-1.5 w-1.5 rounded-full flex-shrink-0 ${ls.open ? 'bg-emerald-400' : 'bg-neutral-400'}`} />
                            <span className="text-[9px] font-bold text-white">{ls.label}</span>
                          </div>
                        ); })()}
                      </div>
                      <div className="flex-1 min-w-0 py-1.5 flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-0.5">
                          ...
                          <span className="text-[13px] font-bold ml-1" style={{ color: dm ? '#d1d5db' : '#404040' }}>{biz.rating}</span>
                        </div>
                        <p className="text-[12px]" style={{ color: dm ? '#6b7280' : '#9ca3af' }}>({biz.reviews} reviews)</p>
                      </div>
                    </button>
                  ))}
                </div>
