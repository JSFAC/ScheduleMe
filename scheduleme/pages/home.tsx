...
function ReferCard() {
  ...
  if (!open) return (
    <div className="rounded-2xl border border-neutral-200 bg-white px-5 sm:px-6 py-4.5 sm:py-5 flex card-elevated items-center gap-3.5 sm:gap-4" style={{ marginLeft: 'max(24px, calc((100vw - 1400px) / 2))', marginRight: 'max(24px, calc((100vw - 1400px) / 2))' }}>
      ...
      <div className="flex-1 min-w-0 pr-1">
        <p className="text-sm font-bold text-neutral-900 leading-tight">Know a skilled student?</p>
        <p className="text-xs text-neutral-500 mt-0.5 leading-relaxed">Refer a trusted campus provider and we will invite them to join.</p>
      </div>
      <button onClick={() => setOpen(true)}
        className="shrink-0 text-xs font-black text-accent border border-accent/25 bg-accent-light hover:brightness-95 px-4 py-2.5 rounded-xl transition-colors tracking-widest uppercase">
        Refer
      </button>
    </div>
  );
