...
export default function Hero({
  ...
  const { dm } = useDm();
  return (
  <section className="relative overflow-hidden pt-24 pb-20 md:pt-36 md:pb-32" style={{ background: dm ? '#0a0a0a' : '#FCFAF6' }}>
      ...
      <div
        ...
        style={{
          background:
          'radial-gradient(circle, rgba(0,126,109,0.10) 0%, transparent 70%)',
        }}
      />
          ...
          <div className="p-1 shadow-modal rounded-2xl" style={{ background: dm ? '#171717' : 'white', border: dm ? '1px solid #262626' : '1px solid rgba(0,0,0,0.07)' }}>
            <div className="rounded-2xl text-left space-y-3 p-5" style={{ background: dm ? '#0f0f0f' : '#f5f5f7', border: dm ? '1px solid #1c1c1e' : '1px solid rgba(0,0,0,0.06)' }}>
              ...
              <div className="flex items-end gap-2.5 justify-end">
                <div className="rounded-2xl rounded-br-sm px-4 py-2.5 text-sm text-white max-w-[220px] text-center" style={{ background: '#007e6d' }}>
                  <p className="leading-snug">I NEED A HAIRCUT ASAP someone whos good at bangs</p>
                </div>
                <div className="h-7 w-7 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold text-white" style={{ background: '#007e6d' }}>J</div>
              </div>
              {/* System match result */}
                  ...
                  <div className="space-y-1.5">
                    {[{ name: 'Mike R.', tag: 'Plumbing', rating: '4.9', time: '~30 min' }, { name: 'Sarah K.', tag: 'Plumbing', rating: '4.8', time: '~1 hr' }].map((pro,i) => (
                      <div key={i} className="flex items-center justify-between gap-3 px-3 py-2 rounded-xl" style={{ background: dm ? '#2c2c2e' : '#f5f5f7' }}>
                        <div className="flex items-center gap-2">
                          <div className="h-6 w-6 rounded-full flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0" style={{ background: '#007e6d' }}>{pro.name[0]}</div>
                          <div>
                        ...
                        <div className="text-right">
                          <p className="text-[10px] font-bold" style={{ color: '#007e6d' }}>★ {pro.rating}</p>
              ...
              <div className="grid grid-cols-3 gap-2 pt-2">
                {[{label:'Verified Pro',color:'#007e6d'},{label:'Top Rated',color:'#007e6d'},{label:'Nearby',color:'#007e6d'}].map((item) => (
